...
public class SkiaViewPresenter : IViewPresenter
{
    private GL _gl;
    private uint _vao;
    private uint _vbo;
    ...
    public SkiaViewPresenter()
    {
        
    }
    
    public unsafe void Initialize(IGraphicsPlatform graphicsPlatform)
    {
        var view = graphicsPlatform.GetNativeWindow()!;
        _gl = graphicsPlatform is IGlGraphicsPlatform platform 
            ? platform.Gl 
            : throw new InvalidOperationException($"Invalid graphics platform - \"{graphicsPlatform}\"");
    ...
    void main()
    {
        ...
        InitializeCanvas(view.Size.X, view.Size.Y);
        ...
        fixed (byte* ptr = _bitmap.GetPixelSpan())
        {
            ...
            _gl.TexImage2D(TextureTarget.Texture2D, 0, InternalFormat.Rgba, (uint) _bitmap.Width, 
                (uint) _bitmap.Height, 0, PixelFormat.Rgba, PixelType.UnsignedByte, ptr);
        }
    ...
    public unsafe void Present(double dt, IVisualTree visualTree)
    {
        ...
        using var blob = SKTextBlob.Create($"text test {DateTime.Now.TimeOfDay:G}".AsSpan(), skFont);
        ...
        _gl.TextureParameter(_texture, TextureParameterName.TextureMaxLevel, 0);
        _gl.GenerateMipmap(TextureTarget.Texture2D);
